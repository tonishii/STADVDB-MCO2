...
export async function readTitleWithDelay(tconst: string, delaySeconds: number) {
  ...
  if (currentNodeEnv.includes("Node 1")) searchOrder = [NODES.NODE1, NODES.NODE0, NODES.NODE2];
  else if (currentNodeEnv.includes("Node 2")) searchOrder = [NODES.NODE2, NODES.NODE0, NODES.NODE1];
  ...
  for (const nodeConfig of searchOrder) {
    ...
    try {
      ...
      if (rows.length > 0) {
        const locationTag = nodeConfig.name.includes(currentNodeEnv.split(" ")[1]) 
          ? "(Local Hit)" 
          : "(Routed)";
...
export async function updateAttributeWithDelay(
  ...
  if (rows.length === 0) return { success: false, logs: ["Item not found in Central Directory"] };
  
  const item = rows[0];
  const target = getTargetNode(item.startYear);
  
  if (!target) return { success: false, logs: [`Item year ${item.startYear} is outside our partition range`] };
  ...
  try {
    ...
    await logTransaction({ 
        id: crypto.randomUUID(), transactionId: txId, timestamp: new Date().toISOString(), 
        node: currentNode as any, operation: "START" 
    });
    ...
    const updateSQL = (table: string) => `UPDATE ${table} SET ${column} = ? WHERE tconst = ? AND SLEEP(?) = 0`;

    logs.push(`Writing to Central Node...`);
    await cCentral.query(updateSQL("node0_titles"), [newValue, tconst, delaySeconds]);
    ...
    await logTransaction({ 
        id: crypto.randomUUID(), transactionId: txId, timestamp: new Date().toISOString(), 
        node: currentNode as any, operation: "UPDATE", values: { tconst, [column]: newValue } 
    });
    ...
    await logTransaction({ 
        id: crypto.randomUUID(), transactionId: txId, timestamp: new Date().toISOString(), 
        node: currentNode as any, operation: "COMMIT" 
    });
    ...
    await logTransaction({ 
        id: crypto.randomUUID(), transactionId: txId, timestamp: new Date().toISOString(), 
        node: currentNode as any, operation: "ABORT" 
    });
